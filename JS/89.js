...
function solution(want, number, discount) {
  ...
  // 10일 동안 원하는 제품이 모두 할인을 받을 수 있어야 함(근데 왜 할인 제품을 하루에 하나씩 삼? 수량 제한이 있나)
  let wantMap = new Map();

  want.forEach((element, index) => {
    wantMap.set(element, number[index]);
  });

  // discount 배열을 10개 단위로 잘라서 제품의 수량을 뽑기
  for (let i = 0; i <= discount.length - 10; i++) {
    const windowItems = discount.slice(i, i + 10);
